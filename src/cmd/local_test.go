...
func TestAPairingOfferIsThreeFields(t *testing.T) {
	code, as, kind, err := offerAsked("abcd-efgh-ijkl bob person")
	if err != nil {
		t.Fatalf("a well-formed line was refused: %v", err)
	}
	if code != "abcd-efgh-ijkl" || as != "bob" || kind != offerPerson {
		t.Errorf("read %q, %q, %v", code, as, kind)
	}
	...
	if _, _, kind, err = offerAsked("abcd-efgh-ijkl - mine"); err != nil || !kind.mine() {
		t.Errorf("mine came out %v (%v)", kind, err)
	}
