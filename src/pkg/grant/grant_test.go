...
func TestARefusalBeatsTheConfig(t *testing.T) {
	...
	table := ns.NewTable()
	if err := table.Add(ns.Mount{Path: "/work", Kind: ns.KindChat, Access: ns.Access{Named: []string{"bob"}}}); err != nil {
		t.Fatal(err)
	}
	table.Granted(s)
...
func TestARefusalNamesABareID(t *testing.T) {
	...
	table := ns.NewTable()
	if err := table.Add(ns.Mount{Path: "/wide", Kind: ns.KindChat, Access: ns.Access{Anyone: true}}); err != nil {
		t.Fatal(err)
	}
	table.Granted(s)
