...
| **Adding** | one *Add* button, for every device: *Nearby* lists the drops on this network, and tapping one asks it — its person says yes, with the same number on both screens. Anywhere else, *My code* shows a QR and a short code, and *Their code* scans or takes the one the other device shows (`drop add` on a computer). A `drop://pair/…` link opens the same way |
| **Making it yours** | on a device you added, *Make it one of my machines* — or *Join their machines*, to make this phone one of theirs. Its person says yes on its screen. A phone wearing your badge has a computer of yours that holds your key do the asking. See [pairing](../../docs/pairing.md#adding-a-device-and-making-it-yours) |
