...
| **People, then machines, then topics** | the same arrangement as the terminal interface: who, which of their machines, and the topics on it — a chat, a folder, an inbox, a note, a terminal |
| **Adding** | *Add a person* on the first screen: *Nearby* lists the drops on this network, and tapping one asks it — its person says yes, with the same number on both screens. Anywhere else, *My code* shows a QR and a short code, and *Their code* scans or takes the one the other device shows (`drop person add` on a computer). A `drop://pair/…` link opens the same way |
| **Your key, and your machines** | your own screen says which key you are — its fingerprint, and whether it is on this phone, in a YubiKey, or on another machine of yours. *Change* makes it your YubiKey, held to the phone with its PIN, or an SSH key file. Every machine of yours holding the same key finds the phone by itself, with no code. *Add a machine* is the other two ways: scan or type the code `drop machine add` shows on a computer of yours, or, on a phone that holds your key, show one for a computer to take |
| **Topics** | on any machine of yours, this phone included, *Add a topic*: a name and a kind, and it is there, open only to you until you say otherwise. The ⋮ on each one says *Who may open it* and *Remove* |
| **Making it yours** | on a device you added, *Make it one of my machines* — or *Join their machines*, to make this phone one of theirs. Its person says yes on its screen. A phone wearing your badge has a computer of yours that holds your key do the asking. See [pairing](../../docs/pairing.md#adding-a-person-and-adding-a-machine) |
| **Your YubiKey** | when your key is a YubiKey, the phone signs with it too: *Make it one of my machines* asks you to hold the YubiKey to the back of the phone, or plug it in, and one tap signs. Nothing to set up — the phone learns which credential on the key is yours from your other machines — and the key never leaves the YubiKey |
